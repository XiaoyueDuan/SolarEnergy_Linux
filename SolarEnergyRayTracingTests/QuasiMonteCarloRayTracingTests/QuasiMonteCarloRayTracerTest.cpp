...
TEST_F(QMCRTracerTestFixture, setFlatRectangleHeliostatVertexes) {
    ...
    Grid *rectGrid = solarScene->getGrid0s()[0];
    int start_id = rectGrid->getStartHeliostatPosition();
...
TEST_F(QMCRTracerTestFixture, generateHeliostatArgument) {
    ...
    std::vector<float3> microHelioOrigins = deviceArray2vector(heliostatArgument.d_microHelio_origins,
                                                               heliostatArgument.numberOfMicroHeliostats);
    std::vector<float3> microHelioNormals = deviceArray2vector(heliostatArgument.d_microHelio_normals,
                                                               heliostatArgument.numberOfMicroHeliostats);

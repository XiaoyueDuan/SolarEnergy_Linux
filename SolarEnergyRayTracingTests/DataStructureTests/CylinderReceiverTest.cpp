...
TEST_F(CylinderReceiverFixture, hasIntersection) {
    ...
    EXPECT_TRUE(cylinderReceiver.GIntersect(origin1, dirZPositive, ans.x, ans.y, ans.z, normal));
    EXPECT_TRUE(Float3Eq(make_float3(6 - 1.5 * sqrtf(3), 0.5f, 5.0f / 6.0f), ans, 1e-4));   // expect u: 0.5, v: 5/6
    EXPECT_TRUE(Float3Eq(normalize(make_float3(1.0f, 0.0f, -sqrtf(3))), normal, 1e-4));

    EXPECT_TRUE(cylinderReceiver.GIntersect(
            origin1, normalize(make_float3(-1.0f, 0.0f, 2.0f)), ans.x, ans.y, ans.z, normal));
    EXPECT_TRUE(Float3Eq(make_float3(1.5 * sqrtf(5), 0.5f, 0.75f), ans, 1e-4));             // expect u: 0.5, v: 0.75
    EXPECT_TRUE(Float3Eq(make_float3(0.0f, 0.0f, -1.0f), normal, 1e-4));

    // 2. Origin 2
    float3 origin2 = make_float3(-6.0f, 0.0f, -1.5f);
    float3 dirXPositive = make_float3(1.0f, 0.0f, 0.0f);
    EXPECT_TRUE(cylinderReceiver.GIntersect(origin2, dirXPositive, ans.x, ans.y, ans.z, normal));
    EXPECT_TRUE(
            Float3Eq(make_float3(6 - 1.5 * sqrtf(3), 0.5f, 7.0f / 12.0f), ans, 1e-4));      // expect u: 0.5, v: 7/12
    EXPECT_TRUE(Float3Eq(normalize(make_float3(-sqrtf(3), 0.0f, -1.0f)), normal, 1e-4));

    // 3. Origin 3
    float3 origin3 = make_float3(3.0f, 0.0f, 3.0f);
    float3 dirXNegetive = make_float3(-1.0f, 0.0f, 0.0f);
    EXPECT_TRUE(cylinderReceiver.GIntersect(origin3, dirXNegetive, ans.x, ans.y, ans.z, normal));
    ...
    EXPECT_TRUE(cylinderReceiver.GIntersect(origin3, dir2Point, ans.x, ans.y, ans.z, normal));
    ...
    EXPECT_TRUE(Float3Eq(normalize(make_float3(1.0f, 0.0f, 1.0f)), normal, 1e-4));
}
...
TEST_F(CylinderReceiverFixture, noIntersection) {
    ...
    float3 dirXPositive = make_float3(1.0f, 0.0f, 0.0f);
    EXPECT_FALSE(cylinderReceiver.GIntersect(inner_origin1, dirXPositive, ans.x, ans.y, ans.z, normal));
    ...
    EXPECT_FALSE(cylinderReceiver.GIntersect(outter_origin2, dirXPositive, ans.x, ans.y, ans.z, normal));
    EXPECT_FALSE(cylinderReceiver.GIntersect(outter_origin2, dirPositive, ans.x, ans.y, ans.z, normal));
    ...
    EXPECT_FALSE(cylinderReceiver.GIntersect(outter_origin3, dir, ans.x, ans.y, ans.z, normal));
}

...
public:
    ...
    float3 microHeliostatPointTransfer(float3 position, float3 microhelioLocalNormal, float3 microhelioLocalPosition) {
        position = global_func::local2world(position, microhelioLocalNormal);
        position = global_func::transform(position, microhelioLocalPosition);
        position = global_func::local2world(position, focusFlatRectangleHelio.getNormal());
        return global_func::transform(position, focusFlatRectangleHelio.getPosition());
    }

    float3 microHeliostatPointLocalTransfer(float3 position, float3 microhelioLocalNormal, float3 microhelioLocalPosition) {
        position = global_func::local2world(position, microhelioLocalNormal);
        position = global_func::transform(position, microhelioLocalPosition);
        return global_func::local2world(position, focusFlatRectangleHelio.getNormal());
    }

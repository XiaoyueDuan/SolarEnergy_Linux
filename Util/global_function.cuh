...
namespace global_func {
    ...
    __host__ __device__
    inline float3 local2world(const float3 &d_local, const float3 &aligned_normal) {
        // u : X
        // n is normal: Y
        // v : Z
    ...
    template<typename T>
    __host__ __device__
    inline T transform(const T &d_in, const T transform_vector) {
    ...
    __host__ __device__
    inline bool rayParallelogramIntersect(

#ifndef SOLARENERGYRAYTRACING_FOCUSFLATRECTANGLEHELIO_CUH
#define SOLARENERGYRAYTRACING_FOCUSFLATRECTANGLEHELIO_CUH
...
namespace focusFlatRectangle_heliostat {
    __host__ __device__
    inline float3 focusFlatRectangleHeliostatLocal2World(const float3 &d_local, const float3 &aligned_normal) {
        // u : X
        // n is normal: Y
        // v : Z
...
public:
    ...
    void setGPUocalCenters(float3 *h_local_centers);
    void setGPUocalCenters(std::vector<float3> local_centers);
};

#endif //SOLARENERGYRAYTRACING_FOCUSFLATRECTANGLEHELIO_CUH;

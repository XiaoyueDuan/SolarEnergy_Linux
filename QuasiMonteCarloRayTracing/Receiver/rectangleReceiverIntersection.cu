...
__device__ void rectangleReceiverIntersect::receiver_drawing(RectangleReceiver &rectangleReceiver, const float3 &orig,
    ...
    int address = row_col.x * rrectangleReceiver.getResolution().x + row_col.y;  //col_row.y + col_row.x*resolution.y;
    atomicAdd(&(rectangleReceiver.getDeviceImage[address]), energy);
}